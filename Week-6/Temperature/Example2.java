// Find and display the cities which have the lowest minimum temperature in January in their own region.
public class Example2 {
    public static void main(String[] args) {
        ...
        for (int i=0; i<2; i++) {
            double min=temperature[i] [0] [2] [0];
            for(int j=0; j<9; j++)
                if (min > temperature[i] [j][2] [0])
                    min = temperature[i] [j][2] [0];
            
            for(int j=0; j<9; j++)
                if (min == temperature[i] [j][2] [0]) 
                    System.out.printf ("%s has the lowest minimum temperature in January in %s.", dim3[i][j], dim4[i]);
